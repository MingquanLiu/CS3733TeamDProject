...
public class AStar implements PathStrategyIF {
     ...
     */
    public AStar(List<NodeData> nodes, List<Edge> edges, String startID, String goalID) throws NoPathException {
        this.allEdges = edges;
        this.allNodes = nodes;
        this.init();
...
//        return null;
    ...
    private double distanceToGo(StarNode node, StarNode goal) {
        ...
        if(node.getType().equals("STAI")) {
            distToGo = 2 * distToGo;// * Math.abs(Integer.parseInt(node.getFloor()) - Integer.parseInt(goal.getFloor()));
        }
        else if(node.getType().equals("ELEV")) {
            distToGo = 0.25 * distToGo;// * Math.abs(Integer.parseInt(node.getFloor()) - Integer.parseInt(goal.getFloor()));
        }
        return distToGo;
    }
